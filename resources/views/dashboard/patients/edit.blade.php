    ...
    <form action="{{ route('dashboard.patients.update', $patient) }}" method="POST" class="patient-form" enctype="multipart/form-data">
        ...
        <div class="patient-form-top">
            <div class="form-group form-group-foto">
                ...
                <div class="foto-upload-zone" id="fotoPreviewWrap" role="button" tabindex="0" aria-label="Pilih foto">
                    <div class="foto-placeholder" id="fotoPlaceholder" style="{{ $patient->foto_url ? 'display:none;' : '' }}">
                        <span class="foto-placeholder-icon">👤</span>
                        <span class="foto-placeholder-text">Klik untuk ganti foto</span>
                        <span class="foto-placeholder-hint">JPG, PNG atau WebP • Maks. 2 MB</span>
                    </div>
                    <img id="fotoPreview" class="foto-preview-img" src="{{ $patient->foto_url ? asset('storage/' . $patient->foto) : '' }}" alt="" style="{{ $patient->foto_url ? '' : 'display:none;' }}">
                </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap', $patient->nama_lengkap) }}" required maxlength="255" placeholder="Contoh: Yohanes Daeli">
                @error('nama_lengkap')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="tempat_lahir">Tempat Lahir</label>
                <input type="text" id="tempat_lahir" name="tempat_lahir" value="{{ old('tempat_lahir', $patient->tempat_lahir) }}" maxlength="100" placeholder="Contoh: Mimika, Jayapura">
                @error('tempat_lahir')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="tanggal_lahir">Tanggal Lahir</label>
                <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir', $patient->tanggal_lahir?->format('Y-m-d')) }}">
            ...
            <div class="form-group">
                ...
                <input type="date" id="tanggal_masuk" name="tanggal_masuk" value="{{ old('tanggal_masuk', $patient->tanggal_masuk->format('Y-m-d')) }}" required>
            ...
            <div class="form-group">
                ...
                <select id="status" name="status" required>
                    ...
                    <option value="dirujuk" {{ old('status', $patient->status) === 'dirujuk' ? 'selected' : '' }}>Dirujuk</option>
                </select>
                @error('status')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group form-group-tanggal-keluar" id="wrapTanggalKeluar" style="{{ old('status', $patient->status) === 'selesai' ? '' : 'display:none;' }}">
                <label for="tanggal_keluar">Tanggal Keluar <span class="required" id="labelRequiredTanggalKeluar">*</span></label>
                <input type="date" id="tanggal_keluar" name="tanggal_keluar" value="{{ old('tanggal_keluar', $patient->tanggal_keluar?->format('Y-m-d')) }}">
        ...
        <div class="form-group form-group-deskripsi">
            ...
            <textarea id="deskripsi" name="deskripsi" rows="4" maxlength="2000" placeholder="Ringkasan kondisi, riwayat, atau catatan tambahan tentang pasien...">{{ old('deskripsi', $patient->deskripsi) }}</textarea>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('dashboard.patients.show', $patient) }}" class="btn btn-outline">Lihat Detail</a>
        </div>
    </form>
</div>
...
(function() {
    ...
    fileInput.addEventListener('change', function() {
        if (this.files && this.files[0]) {
            ...
            r.onload = function() { img.src = r.result; img.style.display = 'block'; if (placeholder) placeholder.style.display = 'none'; };
