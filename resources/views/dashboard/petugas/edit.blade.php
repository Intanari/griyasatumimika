...
<div class="card petugas-form-card">
    <div class="petugas-form-header">
        ...
        <p class="petugas-form-subtitle">Perbarui data identitas, pekerjaan, dan akun sistem.</p>
    </div>

    <form action="{{ route('dashboard.petugas.update', $petuga) }}" method="POST" class="petugas-form" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="petugas-form-grid">
                <div class="form-group form-group-foto">
                    ...
                    <div class="petugas-foto-zone" id="fotoPreviewWrap" role="button" tabindex="0">
                        <div class="petugas-foto-placeholder" id="fotoPlaceholder" style="{{ $petuga->foto_url ? 'display:none;' : '' }}">
                            <span class="petugas-foto-icon">📷</span>
                            <span>Klik ganti foto</span>
                        </div>
                        <img id="fotoPreview" class="petugas-foto-preview" src="{{ $petuga->foto_url ?: '' }}" alt="" style="{{ $petuga->foto_url ? '' : 'display:none;' }}">
                    </div>
                ...
                <div class="petugas-form-fields">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $petuga->name) }}" required maxlength="255">
                    ...
                    <div class="form-group">
                        ...
                        <select id="jenis_kelamin" name="jenis_kelamin">
                            ...
                            <option value="L" {{ old('jenis_kelamin', $petuga->jenis_kelamin) === 'L' ? 'selected' : '' }}>Laki-laki</option>
                            <option value="P" {{ old('jenis_kelamin', $petuga->jenis_kelamin) === 'P' ? 'selected' : '' }}>Perempuan</option>
                        </select>
                        @error('jenis_kelamin')<span class="form-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label for="tempat_lahir">Tempat Lahir</label>
                        <input type="text" id="tempat_lahir" name="tempat_lahir" value="{{ old('tempat_lahir', $petuga->tempat_lahir) }}" maxlength="100">
                        @error('tempat_lahir')<span class="form-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label for="tanggal_lahir">Tanggal Lahir</label>
                        <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir', $petuga->tanggal_lahir?->format('Y-m-d')) }}">
                        @error('tanggal_lahir')<span class="form-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group form-group-full">
                        <label for="alamat">Alamat</label>
                        <textarea id="alamat" name="alamat" rows="2" maxlength="500">{{ old('alamat', $petuga->alamat) }}</textarea>
                        @error('alamat')<span class="form-error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label for="no_hp">Nomor HP</label>
                        <input type="text" id="no_hp" name="no_hp" value="{{ old('no_hp', $petuga->no_hp) }}" maxlength="20">
        ...
        <div class="petugas-form-section">
            ...
            <div class="petugas-form-grid petugas-form-grid-single">
                <div class="form-group">
                    ...
                    <input type="date" id="tanggal_bergabung" name="tanggal_bergabung" value="{{ old('tanggal_bergabung', $petuga->tanggal_bergabung?->format('Y-m-d')) }}">
                ...
                <div class="form-group">
                    ...
                    <select id="status_kerja" name="status_kerja" required>
                        <option value="aktif" {{ old('status_kerja', $petuga->status_kerja) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="cuti" {{ old('status_kerja', $petuga->status_kerja) === 'cuti' ? 'selected' : '' }}>Cuti</option>
                        <option value="nonaktif" {{ old('status_kerja', $petuga->status_kerja) === 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="shift_jaga" name="shift_jaga">
                        <option value="">-- Pilih --</option>
                        <option value="pagi" {{ old('shift_jaga', $petuga->shift_jaga) === 'pagi' ? 'selected' : '' }}>Pagi</option>
                        <option value="siang" {{ old('shift_jaga', $petuga->shift_jaga) === 'siang' ? 'selected' : '' }}>Siang</option>
                        <option value="malam" {{ old('shift_jaga', $petuga->shift_jaga) === 'malam' ? 'selected' : '' }}>Malam</option>
                    </select>
        ...
        <div class="petugas-form-section">
            ...
            <div class="petugas-form-grid petugas-form-grid-single">
                <div class="form-group">
                    ...
                    <input type="text" id="username" name="username" value="{{ old('username', $petuga->username) }}" maxlength="100" placeholder="Opsional">
                ...
                <div class="form-group">
                    ...
                    <input type="password" id="password_confirmation" name="password_confirmation" minlength="8" placeholder="Ulangi jika mengubah">
                </div>
                <div class="form-group">
                    <label for="role">Role <span class="required">*</span></label>
                    <select id="role" name="role" required>
                        <option value="petugas_rehabilitasi" {{ old('role', $petuga->role) === 'petugas_rehabilitasi' ? 'selected' : '' }}>Petugas Rehabilitasi</option>
        ...
        <div class="petugas-form-actions">
            ...
            <a href="{{ route('dashboard.petugas.show', $petuga) }}" class="btn btn-outline">Lihat Detail</a>
        </div>
    </form>
</div>
