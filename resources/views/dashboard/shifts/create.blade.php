...
@section('topbar-title', 'Tambah Shift')
...
<div class="sf-page">
    <nav class="sf-breadcrumb" aria-label="Breadcrumb">
        ...
        <span class="sf-breadcrumb-sep">/</span>
        <span class="sf-breadcrumb-current">Tambah</span>
    </nav>
    ...
    <div class="sf-card">
        <div class="sf-header">
            <div class="sf-header-main">
                ...
                <div>
                    <h2 class="sf-title">Tambah Shift</h2>
                    <p class="sf-subtitle">Waktu mulai dan waktu selesai shift</p>
                </div>
            </div>
        </div>

        <form action="{{ route('dashboard.shifts.store') }}" method="POST" class="sf-form">
            @csrf
            <div class="sf-form-grid">
                <div class="sf-form-group sf-col-full">
                    <label class="sf-label">Nama Shift <span class="sf-required">*</span></label>
                    <input type="text" name="nama" value="{{ old('nama') }}" class="sf-input {{ $errors->has('nama') ? 'sf-invalid' : '' }}" placeholder="Contoh: Pagi, Siang, Malam" required>
                    @error('nama')<span class="sf-error">{{ $message }}</span>@enderror
                </div>
                <div class="sf-form-group">
                    <label class="sf-label">Jam Mulai <span class="sf-required">*</span></label>
                    <input type="time" name="jam_mulai" value="{{ old('jam_mulai') }}" class="sf-input {{ $errors->has('jam_mulai') ? 'sf-invalid' : '' }}" required>
                ...
                <div class="sf-form-group">
                    ...
                    <input type="time" name="jam_selesai" value="{{ old('jam_selesai') }}" class="sf-input {{ $errors->has('jam_selesai') ? 'sf-invalid' : '' }}" required>
                    @error('jam_selesai')<span class="sf-error">{{ $message }}</span>@enderror
                </div>
            </div>
            <div class="sf-form-actions">
                <button type="submit" class="sf-btn-primary">Simpan Shift</button>
...
.sf-header-icon { width: 48px; height: 48px; border-radius: 12px; background: linear-gradient(135deg, rgba(34,197,94,0.12), rgba(16,185,129,0.08)); border: 1px solid rgba(34,197,94,0.2); color: #059669; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
