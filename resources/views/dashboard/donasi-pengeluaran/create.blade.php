...
@section('title', 'Tambah Pengeluaran Donasi')
@section('topbar-title', 'Tambah Pengeluaran Donasi')
...
<nav class="donasi-pengeluaran-breadcrumb" aria-label="Breadcrumb">
    ...
    <span class="donasi-pengeluaran-sep">/</span>
    <span class="donasi-pengeluaran-current">Tambah Pengeluaran</span>
</nav>

<div class="card">
    <div class="card-title">Tambah Pengeluaran Donasi</div>
    <form action="{{ route('dashboard.donasi.pengeluaran.store') }}" method="POST" enctype="multipart/form-data" class="donasi-pengeluaran-form">
        @csrf
        <div class="form-group">
            <label for="keterangan">Digunakan untuk apa <span class="required">*</span></label>
            <input type="text" id="keterangan" name="keterangan" value="{{ old('keterangan') }}" required maxlength="500" placeholder="Contoh: Pembelian obat rawat inap, Bantuan sembako">
            @error('keterangan')<span class="form-error">{{ $message }}</span>@enderror
        </div>
        <div class="form-group">
            <label for="jumlah">Jumlah (Rp) <span class="required">*</span></label>
            <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah') }}" required min="1" placeholder="Nominal pengeluaran">
