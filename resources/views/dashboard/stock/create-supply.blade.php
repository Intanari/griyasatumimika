...
@section('topbar-title', 'Tambah Stok Barang')

@section('content')
<a href="{{ route('dashboard.stock.index') }}" class="page-back-link">Back</a>

<div class="card">
    <div class="card-title">Tambah Stok Barang</div>
    <p class="page-table-desc">Isi nama stok barang, jumlah, harga, dan gambar. Waktu tercatat otomatis saat disimpan.</p>
    <form action="{{ route('dashboard.stock.tambah.store') }}" method="POST" enctype="multipart/form-data" class="stock-form">
        @csrf
        <div class="form-group">
            <label for="nama">Nama stok barang <span class="required">*</span></label>
            <input type="text" id="nama" name="nama" value="{{ old('nama') }}" required maxlength="255" placeholder="Contoh: Beras, Obat A" list="nama-list">
