...
@section('title', 'Detail Tahapan Rehabilitasi')
@section('topbar-title', 'Detail Tahapan Rehabilitasi')
...
<div class="card admin-account-form-card">
    <div class="admin-account-form-header">
        ...
        <div style="flex: 1;">
            <h2 class="admin-account-form-title">{{ $tahapanRehabilitasi->judul }}</h2>
            <p class="admin-account-form-subtitle">
                No urut: {{ $tahapanRehabilitasi->no_urut }}
                @if($tahapanRehabilitasi->status)
                    · Status: {{ $tahapanRehabilitasi->status }}
                @endif
            </p>
        </div>
        <div style="display: flex; gap: 0.5rem;">
            <a href="{{ route('dashboard.layanan.tahapan-rehabilitasi.edit', $tahapanRehabilitasi) }}" class="btn btn-primary btn-sm">Edit</a>
            <form action="{{ route('dashboard.layanan.tahapan-rehabilitasi.destroy', $tahapanRehabilitasi) }}" method="POST" onsubmit="return confirm('Yakin hapus tahapan ini?');">
                @csrf
                @method('DELETE')
    ...
    <div class="layanan-detail-body">
        <div class="layanan-detail-content">{!! nl2br(e($tahapanRehabilitasi->keterangan)) !!}</div>
    </div>
</div>
