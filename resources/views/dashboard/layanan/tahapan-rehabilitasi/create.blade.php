...
@section('title', 'Tambah Tahapan Rehabilitasi')
@section('topbar-title', 'Tambah Tahapan Rehabilitasi')
...
<div class="card admin-account-form-card">
    <div class="admin-account-form-header">
        ...
        <div>
            <h2 class="admin-account-form-title">Tambah Tahapan Rehabilitasi</h2>
            <p class="admin-account-form-subtitle">Isi no urut, status, judul, dan keterangan yang akan ditampilkan di halaman publik Layanan.</p>
        </div>
    </div>

    <form action="{{ route('dashboard.layanan.tahapan-rehabilitasi.store') }}" method="POST" class="admin-account-form">
        @csrf

        <div class="admin-account-section">
            <h3 class="admin-account-section-title"><span class="admin-account-section-icon">#</span> No Urut</h3>
            <div class="form-group">
                <label for="no_urut">No Urut <span class="required">*</span></label>
                <input type="number" id="no_urut" name="no_urut" value="{{ old('no_urut', 1) }}" required min="1" placeholder="1">
                @error('no_urut')<span class="form-error">{{ $message }}</span>@enderror
            </div>
        </div>

        <div class="admin-account-section">
            <h3 class="admin-account-section-title"><span class="admin-account-section-icon">🏷️</span> Status</h3>
            <div class="form-group">
                <label for="status">Status (tag, misal: Penjangkauan, Asesmen Awal)</label>
                <input type="text" id="status" name="status" value="{{ old('status') }}" maxlength="100" placeholder="Contoh: Penjangkauan">
        ...
        <div class="admin-account-section">
            ...
            <div class="form-group">
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required maxlength="255" placeholder="Contoh: Tanggap Laporan dan Survei Lapangan">
        ...
        <div class="admin-account-section">
            ...
            <div class="form-group">
                ...
                <textarea id="keterangan" name="keterangan" rows="5" placeholder="Deskripsi tahapan...">{{ old('keterangan') }}</textarea>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('dashboard.layanan.index') }}" class="btn btn-outline">Batal</a>
        </div>
    </form>
</div>
...
.form-group input, .form-group textarea { width: 100%; padding: 0.6rem 0.85rem; border: 1px solid var(--border); border-radius: 8px; font-size: 0.9rem; color: var(--text); background: var(--card); }
