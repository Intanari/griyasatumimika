...
@section('title', 'Tambah Petugas Yayasan')
@section('topbar-title', 'Tambah Petugas Yayasan')
...
<div class="card admin-account-form-card">
    <div class="admin-account-form-header">
        ...
        <div>
            <h2 class="admin-account-form-title">Tambah Petugas Yayasan</h2>
            <p class="admin-account-form-subtitle">Isi foto, nama, status, dan keterangan petugas yang akan ditampilkan di halaman publik.</p>
        </div>
    </div>

    <form action="{{ route('dashboard.petugas-yayasan.store') }}" method="POST" enctype="multipart/form-data" class="admin-account-form">
        @csrf
        ...
        <div class="admin-account-section">
            ...
            <div class="form-group">
                ...
                <p class="form-hint">Opsional. Maks 2MB, format JPG/PNG/WebP.</p>
        ...
        <div class="admin-account-section">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama') }}" required maxlength="255" placeholder="Nama lengkap">
                @error('nama')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="status">Status / Jabatan</label>
                <input type="text" id="status" name="status" value="{{ old('status') }}" maxlength="255" placeholder="Contoh: Staf Operasional">
                @error('status')<span class="form-error">{{ $message }}</span>@enderror
            </div>
            <div class="form-group">
                <label for="keterangan">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="4" placeholder="Deskripsi singkat tugas atau peran...">{{ old('keterangan') }}</textarea>
