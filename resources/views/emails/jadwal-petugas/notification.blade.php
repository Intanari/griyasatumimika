...
<head>
    ...
    <title>Notifikasi Jadwal Petugas – {{ config('app.name', 'PeduliJiwa') }}</title>
    <style>
...
@php
    $actionLabels = [
        'created' => ['label' => 'Jadwal Petugas Ditambahkan', 'icon' => '✅', 'bg' => 'linear-gradient(135deg, #059669 0%, #10b981 100%);'],
        'updated' => ['label' => 'Jadwal Petugas Diperbarui',  'icon' => '📝', 'bg' => 'linear-gradient(135deg, #2563eb 0%, #3b82f6 100%);'],
        'deleted' => ['label' => 'Jadwal Petugas Dihapus',     'icon' => '🗑',  'bg' => 'linear-gradient(135deg, #dc2626 0%, #ef4444 100%);'],
    ];
    $info = $actionLabels[$action] ?? ['label' => 'Perubahan Jadwal Petugas', 'icon' => '📅', 'bg' => 'linear-gradient(135deg, #4f46e5, #7c3aed);'];
    $jam = '';
    if ($jadwal->jam_mulai) {
        $jam = \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i');
        if ($jadwal->jam_selesai) {
            $jam .= ' – ' . \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i');
        }
    } else {
        $jam = '–';
    }
            ...
            <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px; width:100%;">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background: {{ $info['bg'] }}; border-radius: 24px 24px 0 0; overflow: hidden;">
                            <tr>
                                <td align="center" style="padding: 36px 32px;">
                                    ...
                                    <p style="font-size:12px; color:rgba(255,255,255,0.8); text-transform:uppercase; letter-spacing:0.1em; font-weight:600; margin-bottom:8px;">Jadwal Petugas</p>
                                    ...
                                    <p style="font-size:14px; color:rgba(255,255,255,0.9); margin-top:10px;">
                                        Petugas: <strong>{{ $jadwal->user->name ?? '-' }}</strong>
                                    </p>
                                </td>
                            </tr>
                        </table>
                ...
                <tr>
                    <td style="background:#ffffff; border-radius:0 0 24px 24px; box-shadow:0 20px 60px rgba(79,70,229,0.12); padding:32px 36px;">
                        ...
                        <p style="font-size:14px; color:#1a1a2e; margin-bottom:20px; line-height:1.5;">
                            Kepada <strong>{{ $jadwal->user->name ?? 'Petugas' }}</strong>, berikut informasi terkait jadwal Anda yang {{ $action === 'created' ? 'baru ditambahkan' : ($action === 'updated' ? 'telah diperbarui' : 'telah dihapus') }}.
                        </p>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">
                            <tr>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; color:#8a8aaa; width:160px;">Nama Petugas</td>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; font-weight:600; color:#1a1a2e;">{{ $jadwal->user->name ?? '-' }}</td>
                            </tr>
                            <tr>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; color:#8a8aaa;">Tanggal</td>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; font-weight:600; color:#1a1a2e;">{{ $jadwal->tanggal->locale('id')->translatedFormat('d F Y') }}</td>
                            </tr>
                            <tr>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; color:#8a8aaa;">Shift</td>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; font-weight:600; color:#1a1a2e;">{{ $jadwal->shift_label }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; font-weight:600; color:#1a1a2e;">{{ $jadwal->lokasi ?: '–' }}</td>
                            </tr>
                            <tr>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; color:#8a8aaa; vertical-align:top;">Keterangan</td>
                                <td style="padding:10px 0; border-bottom:1px solid #f0f0f8; color:#1a1a2e; line-height:1.5;">{{ $jadwal->keterangan ?: '–' }}</td>
                            </tr>
                            <tr>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:28px;">
                            <tr>
                                <td align="center">
                                    <a href="https://{{ config('app.admin_domain') }}/dashboard/jadwal-petugas"
                ...
                <tr>
                    <td style="padding:24px 0; text-align:center;">
                        <p style="font-size:12px; color:#aaa;">Email notifikasi otomatis dari sistem {{ config('app.name', 'PeduliJiwa') }} – Jadwal Petugas.</p>
